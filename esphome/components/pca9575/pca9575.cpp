...
const uint8_t OUTPUT_REG = 0x05;
const uint8_t INVERT_REG = 0x01;
const uint8_t CONFIG_REG = 0x03;

static const char *const TAG = "pca9575";

void PCA9575Component::setup() {
  ESP_LOGCONFIG(TAG, "Setting up PCA9575/PCA9575A...");
  this->reg_width_ = 2;
...
void PCA9575Component::dump_config() {
  ESP_LOGCONFIG(TAG, "PCA9575:");
  ESP_LOGCONFIG(TAG, "  I/O Pins: %d", this->pin_count_);
  LOG_I2C_DEVICE(this)
  if (this->is_failed()) {
    ESP_LOGE(TAG, "Communication with PCA9575 failed!");
  }
}

bool PCA9575Component::digital_read(uint8_t pin) {
...
void PCA9575Component::digital_write(uint8_t pin, bool value) {
  if (value) {
...
void PCA9575Component::pin_mode(uint8_t pin, gpio::Flags flags) {
...
bool PCA9575Component::read_inputs_() {
  uint8_t inputs[2];
...
bool PCA9575Component::write_register_(uint8_t reg, uint16_t value) {
  uint8_t outputs[2];
...
float PCA9575Component::get_setup_priority() const { return setup_priority::IO; }

// Run our loop() method very early in the loop, so that we cache read values before
// before other components call our digital_read() method.
float PCA9575Component::get_loop_priority() const { return 9.0f; }  // Just after WIFI

void PCA9575GPIOPin::setup() { pin_mode(flags_); }
void PCA9575GPIOPin::pin_mode(gpio::Flags flags) { this->parent_->pin_mode(this->pin_, flags); }
bool PCA9575GPIOPin::digital_read() { return this->parent_->digital_read(this->pin_) != this->inverted_; }
void PCA9575GPIOPin::digital_write(bool value) { this->parent_->digital_write(this->pin_, value != this->inverted_); }
std::string PCA9575GPIOPin::dump_summary() const {
  char buffer[32];
  snprintf(buffer, sizeof(buffer), "%u via PCA9575", pin_);
  return buffer;
}
